...
public sealed class DemoDataController : ControllerBase
{
    ...
    public DemoDataController(DispatcherDbContext db) => _db = db;
    ...
    [HttpPost("clear")]
    public async Task<ActionResult<DemoDataResult>> Clear(CancellationToken ct)
    {
        var affected = 0;
    ...
    /// </summary>
    [HttpPost("seed")]
    public async Task<ActionResult<DemoDataResult>> Seed(CancellationToken ct)
    {
        if (await _db.Plants.AnyAsync(ct) || await _db.Products.AnyAsync(ct))
        {
            return BadRequest(
                new
                {
                    message = "База не пустая. Сначала выполните сброс (clear).",
                }
            );
        }
        ...
        return Ok(
            new DemoDataResult
            {
                Message = "Демо-данные загружены. Набор правится в DemoDataController.SeedCoreAsync.",
                Affected = count,
            }
        );
    }
